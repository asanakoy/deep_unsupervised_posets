...
class Shufflelearnnet(Convnet):
    """
    Shufflelearnnet. The same network as were used in Shuffle & Learn paper
    ...
    def __init__(self,
        ...
        with tf.device(self.device_id):
            ...
            maxpool1 = tf.nn.local_response_normalization(maxpool1, depth_radius=2,
                                                          alpha=2e-05,
                                                          beta=0.75,
                                                          bias=1.0)
            ...
            maxpool2 = tf.nn.local_response_normalization(maxpool2, depth_radius=2,
                                                          alpha=2e-05,
                                                          beta=0.75,
                                                          bias=1.0)
    ...
    def restore_from_snapshot(self, snapshot_path, num_layers, restore_iter_counter=False):
        ...
        with self.graph.as_default():
            ...
            try:
                ...
                print 'Fallback to restore_from_alexnet_snapshot'
                if restore_iter_counter:
                    raise ValueError('Cannot restore iter counter from alexnet snapshot.')
                self.restore_from_alexnet_snapshot(snapshot_path, num_layers)
